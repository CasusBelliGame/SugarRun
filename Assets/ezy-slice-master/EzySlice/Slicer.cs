...
namespace EzySlice {
    ...
    public sealed class Slicer {
        ...
        public static SlicedHull Slice(GameObject obj, Plane pl, TextureRegion crossRegion, Material crossMaterial) {
            ...
            //MeshRenderer rendererMesh = obj.GetComponent<MeshRenderer>();

            // cannot continue without a proper renderer
            if (renderer == null) {
                SlicedHull slice = Slice2(obj,pl,crossRegion,crossMaterial);
                if(slice != null){
                    return slice;
                }
                Debug.Log(obj.name);
        ...
        public static SlicedHull Slice2(GameObject obj, Plane pl, TextureRegion crossRegion, Material crossMaterial) {
            MeshFilter filter = obj.GetComponent<MeshFilter>();
