    ...
    public sealed class SlicedHull {
        ...
        public GameObject CreateUpperHull(GameObject original, Material crossSectionMat) {
            ...
            if (newObject != null) {
                ...
                //newObject.transform.localScale = original.transform.root.localScale;
                if(original.GetComponent<SkinnedMeshRenderer>() != null){
                    shared = original.GetComponent<SkinnedMeshRenderer>().sharedMaterials;
                    newObject.transform.localScale = original.transform.localScale*original.transform.root.localScale.x;
                    newObject.transform.localPosition = original.transform.root.position;
                    newObject.transform.localRotation = original.transform.root.localRotation;
                }
                if(original.GetComponent<MeshRenderer>() != null){
                    deneme = original.GetComponent<MeshRenderer>().sharedMaterials;
                ...
                if(shared != null){
                    foreach (Material item in shared)
                    {
                        mats.Add(item);
                    }
                }
                if(deneme != null){
                    foreach (Material item in deneme)
                    {
                        mats.Add(item);
                    }
                }
                ...
                if (mesh.subMeshCount == upper_hull.subMeshCount) {
                    ...
                    newObject.GetComponent<Renderer>().sharedMaterials = mats.ToArray();
                ...
                Material[] newShared = new Material[mats.ToArray().Length + 1];

                // copy our material arrays across using native copy (should be faster than loop)
                System.Array.Copy(mats.ToArray(), newShared, mats.ToArray().Length);
                newShared[mats.ToArray().Length] = crossSectionMat;
